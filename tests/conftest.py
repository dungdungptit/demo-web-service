...
def pytest_addoption(parser):
    parser.addoption("--submit-tests", action="store_true", help="Submit tests to Jira")
...
class JiraTestService:
    ...
    def get_issue_info(self, issue_key):
        return requests.get(
            url=self.issue_url + "/" + issue_key, auth=self.auth_string
        ).json()
    ...
    def get_tests_in_issue(self, issue_key):
        ...
        response = requests.get(
            url=self.url + "/testcase/search", params=params, auth=self.auth_string
        ).json()
    ...
    def create_test(self, test_name, issue_key, objective, steps):
        json = {
            ...
            "status": "Approved",
            "objective": objective,
            "testScript": {"type": "PLAIN_TEXT", "text": steps or "Default"},
        }
        response = requests.post(
            url=self.url + "/testcase", json=json, auth=self.auth_string
        )
        if response.status_code != 201:
            raise Exception(
                "Create test return with error status code", response.status_code
            )
    ...
    def delete_test(self, test_key):
        response = requests.delete(
            url=self.url + "/testcase/" + test_key, auth=self.auth_string
        )
        if response.status_code != 204:
            raise Exception(
                "Delete test return with error status code", response.status_code
            )
    ...
    def create_test_cycle(self, name, issue_key, items):
        ...
        response = requests.post(
            url=self.url + "/testrun", json=json, auth=self.auth_string
        )
        if response.status_code != 201:
            raise Exception(
                "Create test cycle return with error status code", response.status_code
            )
...
def each_test_suite(request):
    ...
    if submit_tests:
        ...
        test_service.create_test_cycle(
            cls.ISSUE_KEY + " - " + cls.__name__, cls.ISSUE_KEY, test_cycle_items
        )
